use std::{sync::{atomic::Ordering, Arc}, thread::{self, JoinHandle}};

use parking_lot::Mutex;
use tokio::sync::oneshot;

use crate::{eval::{EvalContext, EvalResult}, runtime::{BlinkVM, GoroutineId, GoroutineScheduler, GoroutineState, GoroutineTask, SchedulerState}, ValueRef};
...
impl SingleThreadScheduler {
    ...
    ) -> Option<GoroutineTask> {  // Return the goroutine if it needs to be suspended
        let mut goroutine = goroutine;
        
        let result = match std::mem::replace(&mut goroutine.state, GoroutineState::Completed) {
            GoroutineState::Ready { task } => {
                task(&mut goroutine.context)
            }
            GoroutineState::WaitingForTokio { mut receiver, resume } => {
