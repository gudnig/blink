...
use crate::runtime::{obj_lock, obj_unlock, BlinkActivePlan, BlinkObjectModel, BlinkSlot, ClosureObject, CompiledFunction, Macro, TypeTag, GLOBAL_MMTK};
...
impl BlinkVM {
    ...
    pub fn alloc_list_or_vector(&self, items: Vec<ValueRef>, capacity: Option<usize>, is_list: bool) -> ObjectReference {
        self.with_mutator(|mutator| {

            let capacity = capacity.unwrap_or(items.len());

            //[length: u32][capacity: u32][array_ptr: ObjectReference]
            let header_size =
            std::mem::size_of::<u32>() +
            std::mem::size_of::<u32>() +
            ...
            unsafe {
                ...
                for item in &items {
                    std::ptr::write_unaligned(data_ptr.add(array_offset) as *mut ValueRef, *item);
                    array_offset += std::mem::size_of::<ValueRef>();
                }
            }
            data_start
        })
    ...
    pub fn resize_vector(&self, vector: ObjectReference, new_size: u32) {
        self.with_mutator(|mutator| {
            let total_size = new_size as usize * std::mem::size_of::<ValueRef>();

            obj_lock(vector);
            unsafe {
                let data_ptr = vector.to_raw_address().as_usize() as *mut u8;
                let mut offset = 0;

                let current_size = std::ptr::read_unaligned(data_ptr.add(offset) as *mut u32);

                // If the new size is smaller than the current size, only copy up to the new size
                let copy_to = if new_size >= current_size { current_size } else { new_size };

                
                // Write barrier
                let new_data_ref = BlinkActivePlan::alloc(mutator, &total_size);
                mutator.barrier().object_reference_write(vector, BlinkSlot::ObjectRef(new_data_ref.to_raw_address()), vector);
                ...
                for i in 0..copy_to {
                    ...
                    std::ptr::write_unaligned(new_data_ptr.add(i as usize * std::mem::size_of::<ValueRef>()) as *mut ValueRef, std::ptr::read_unaligned(item_ptr as *mut ValueRef));
                }

                std::ptr::write_unaligned(data_ptr.add(offset) as *mut ObjectReference, new_data_ref);
                offset += std::mem::size_of::<ObjectReference>();

                std::ptr::write_unaligned(data_ptr.add(offset) as *mut u32, new_size as u32);
    ...
    pub fn update_element(&self, vector: ObjectReference, index: u32, element: ValueRef) -> Result<(), ()> {
        self.vector_get_length();
        self.with_mutator(|mutator| {
            ...
            unsafe {
                ...
                let size = std::ptr::read_unaligned(header_ptr.add(offset) as *mut u32);
                offset += std::mem::size_of::<u32>();

                let capacity = std::ptr::read_unaligned(header_ptr.add(offset) as *mut u32);
                offset += std::mem::size_of::<u32>();
                ...
                let data_ref = std::ptr::read_unaligned(header_ptr.add(offset + std::mem::size_of::<u32>()) as *mut ObjectReference);
                ...
                mutator.barrier().object_reference_write(vector, BlinkSlot::ValueRef(Address::from_mut_ptr(item_ptr)), data_ref);
