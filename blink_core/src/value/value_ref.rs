...
use crate::{
    collections::ValueContext, env::Env, error::BlinkError, eval::{EvalContext, EvalResult}, runtime::{ContextualBoundary, SharedArena, ValueBoundary}, value::{is_bool, is_number, is_symbol, pack_bool, pack_keyword, pack_nil, pack_number, pack_symbol, unpack_immediate, GcPtr, HeapValue, ImmediateValue, IsolatedValue}
};
...
impl ValueRef {
    ...
    pub fn is_string(&self, arena: &SharedArena) -> bool {
        match self {
            ValueRef::Shared(idx) => arena
                .get(*idx)
                .map(|v| matches!(v.as_ref(), HeapValue::Str(_)))
                .unwrap_or(false),
            _ => false,
        }
    }
